...
const Toast: React.FC<ToastProps> = ({ message, type, isVisible, onClose }) => {
  ...
  const baseClasses = "fixed top-4 right-4 z-50 p-4 rounded-lg shadow-xl max-w-sm transition-all duration-300";
  const typeClasses = {
  ...
  return (
    <div className={`${baseClasses} ${typeClasses[type]} ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
      ...
      <div className="absolute bottom-0 left-0 h-1 bg-black/20 w-full">
        <div className="h-full bg-black/30 animate-pulse"></div>
      </div>
    </div>
  );
};
