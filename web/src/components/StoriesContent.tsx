...
import { useStories, UseStoriesOptions } from '@/hooks/useStories';
...
const StoriesContent: React.FC = () => {
  ...
  // Create options for useStories hook
  const storiesOptions: UseStoriesOptions = {
  ...
  const { stories, loading, error } = useStories(storiesOptions);
      ...
      <div className="flex justify-between items-center bg-white/70 backdrop-blur-sm rounded-full px-6 py-3 shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <p className="font-medium text-gray-700">
            {stories.length > 0 
              ? `${stories.length} epic ${stories.length === 1 ? 'story' : 'stories'} found!`
      ...
      {stories.length > 0 ? (
            ...
            >
              ...
              <div className="bg-gradient-to-br from-purple-400 via-pink-400 to-orange-400 rounded-3xl p-1 shadow-lg hover:shadow-xl hover:shadow-purple-500/20 transition-all duration-300">
                <div className="bg-white rounded-3xl overflow-hidden h-full">
                  ...
                  <div className="relative story-card-landscape overflow-hidden">
                    <SimpleImage
                      imagePath={story.imageUrl || story.image || 'default.jpg'}
                      alt={story.title}
                      className="story-cover-image w-full h-full group-hover:scale-110 transition-transform duration-500"
                      category="stories"
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-600 text-lg mb-8 max-w-md mx-auto leading-relaxed">
            {appliedFilters.search || appliedFilters.category || appliedFilters.featured
              ? 'Hmm... try different filters? Maybe something amazing is waiting! 🔍'
              : 'Be the first to drop some travel wisdom! Your adventure deserves to be shared ✨'
            }
          </p>
          {(appliedFilters.search || appliedFilters.category || appliedFilters.featured) && (
            <button
              onClick={() => handleFilterChange({ search: '', category: '', featured: false })}
              className="bg-gradient-to-r from-purple-500 to-pink-500 text-white px-8 py-3 rounded-full font-bold hover:from-purple-600 hover:to-pink-600 transition-all duration-300 transform hover:scale-105 shadow-lg"
            >
              🧹 Clear All Filters
            </button>
          )}
